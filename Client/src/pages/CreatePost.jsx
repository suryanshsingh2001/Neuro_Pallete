...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch("https://neuropallete-backend.onrender.com/api/v1/dalle", { //http://localhost:8080/api/v1/dalle
          method: "POST",
          headers: {
      ...
      alert("Looks like you haven't typed anything");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        const response = await fetch("https://neuropallete-backend.onrender.com/api/v1/post", { //http://localhost:8080/api/v1/post
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
    ...
    <section className="max-w-7xl mx-auto">
      <div>
        ...
        <p className="mt-2 text-[#666e75] text-[14px] max-w-[500px] ">
          Showcase the power of AI and share your unique creation through DALL-E
          AI.
        </p>
      </div>
      ...
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        ...
        <div className="mt-10">
          <p className="mt-2 text-[#666e75] text-[14px]">
            Join the conversation - post your machine-generated art for all to
            see
          </p>
          <button
            type="submit"
